export const meta = {
  name: 'lane-a-census-v2-intel',
  description: 'Ownership-census web research on remaining IL GP locations (Lane A v2: census + opportunistic job-hunt intel capture)',
  phases: [
    { title: 'Research', detail: 'one Sonnet 5 agent per 16-practice unit, web evidence only, opportunistic intel rider', model: 'claude-sonnet-5' },
...
const RESEARCH_SCHEMA = {
  type: 'object',
  required: ['unit_id', 'n_total', 'n_classified', 'n_undetermined', 'n_intel', 'result_file', 'tier_tally', 'dso_claims'],
  properties: {
    unit_id: { type: 'string' },
    n_total: { type: 'integer' },
    n_classified: { type: 'integer' },
    n_undetermined: { type: 'integer' },
    n_intel: { type: 'integer' },
...
OPPORTUNISTIC INTEL (secondary mission — job-hunt directory enrichment; classification stays the graded mission):
While researching ownership you will often land on the practice's own website, its Google/review listing, or a careers page anyway. For each practice where you SAW such material, add an OPTIONAL "intel" object to that practice's row, capturing ONLY facts you actually observed on pages you loaded or search-result snippets you read:
{"website_url" (the practice's own site), "website_note" (1-2 sentences: site quality/era, online booking, positioning), "services" (list of strings, e.g. implants/Invisalign/sedation), "technology" (list, e.g. CEREC/iTero/CBCT/laser), "provider_count_web" (int — dentists shown on the site), "provider_notes" (names/roles, dental school + grad year if shown), "owner_career_stage" ("early_career"|"mid_career"|"late_career" — ONLY if grounded in a stated grad year, founding year, or bio), "google_rating" (float), "google_review_count" (int), "google_source_url" (REQUIRED if either google field is present), "hiring" (short note of any job opening you saw), "hiring_source_url" (REQUIRED if hiring present), "accepts_medicaid" (bool — only if explicitly stated), "insurance_note" (short), "distinguishing_notes" (1-2 sentences on what makes this office distinct: ops count, languages spoken, hours, new build, awards), "sources" (REQUIRED: list of the real http(s) URLs these facts came from)}.
INTEL HARD RULES:
   - Intel NEVER justifies extra searches — it rides on the 1-3 ownership searches only. If you saw nothing beyond ownership evidence, OMIT the "intel" key entirely.
   - Every intel object MUST carry >=1 real URL in "sources". Google/hiring fields additionally need their specific source URL, or leave those fields out.
   - Omission over guessing — never estimate ratings, review counts, provider counts, or career stages. Include only fields you can point at a page for.
   - Works for ANY status — an undetermined-ownership practice can still carry intel if you saw its website.

FINAL: return the structured summary (unit_id, counts including n_intel = number of rows carrying an intel object, tier_tally, result_file path, and dso_claims = one entry per row you classified stealth_dso or branded_dso, with claim_summary = 1 sentence naming the brand/controller and the key URL).`
}
...
const totals = { units_ok: done.length, units_failed: results.length - done.length, classified: 0, undetermined: 0, intel: 0, dso_claims: 0, confirmed: 0, refuted_or_downgraded: 0 }
const tierTotals = {}
for (const r of done) {
  totals.classified += r.n_classified
  totals.undetermined += r.n_undetermined
  totals.intel += r.n_intel || 0
...
log(`Wave complete: ${totals.classified} classified / ${totals.undetermined} undetermined / ${totals.intel} intel captures across ${totals.units_ok} units`)
return { totals, tierTotals, perUnit: done.map(r => ({ unit: r.unit_id, file: r.result_file, classified: r.n_classified, undetermined: r.n_undetermined, intel: r.n_intel || 0, verdicts: (r.verdicts || []).map(v => ({ lid: v.location_id, verdict: v.verdict, notes: v.notes })) })) }
