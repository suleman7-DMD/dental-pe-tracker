...
| 10 | **1,178** | corp NPIs | 13,818 watched | `entity_classification IN (dso_regional,dso_national)` (was 1,089 pre-Phase-4 Data-Axle, 875 pre-IL-seed); **Supabase SYNCED 2026-06-10** (670 dso_regional + 508 dso_national, surgical heal) | NPI-row corporate KPI = 8.53% (SQLite = live) |
| 11 | **285** | corp locations | 4,970 GP watched | `zip_scores.corporate_location_count`; was 262 (pre Phase-4 Data-Axle), 200 (pre IL-DSO-seed), 225 (pre-reclass). **Synced — live (2026-06-07).** | Headline corporate share = **5.73%** (floor) |
...
**Watched-ZIP `entity_classification` breakdown (NPI rows, SQLite post-2026-06-07 Phase-4 Data-Axle):** small_group 2,762 / solo_established 2,630 / large_group 2,163 / specialist 1,419 / family_practice 1,316 / solo_high_volume 854 / non_clinical 742 / dso_regional 670 / org_only_npi 572 / dso_national 508 / solo_inactive 165 / solo_new 17. NULL=0. Corporate NPIs = 670+508 = **1,178** in SQLite (was 1,089 pre-Phase-4; the +89 Phase-4 flips came out of small/solo/large/family/org_only). **Supabase SYNCED 2026-06-10** — the 89 stale rows were healed surgically after root-fixing `reclassify_verified_corporate_il.py` (its raw-SQL `practices` UPDATE didn't bump `updated_at`, bypassing the ORM `onupdate`); CI guard `FLOOR_NPI` (expect_min=1178) now watches live.

**Location-level breakdown (`practice_locations`, watched, SQLite post-2026-06-07 Phase-4 Data-Axle — synced to Supabase):** solo_established 2,445 / small_group 922 / specialist 644 / solo_high_volume 638 / large_group 311 / family_practice 203 / dso_national 194 / solo_inactive 167 / dso_regional 91 / non_clinical 23 / solo_new 19. Total = 5,657. Corporate locations = 194+91 = **285** (was 262 pre-Phase-4; the +23 Data-Axle structural-verified promotions all came out of GP-independent classes). Independent (7 classes) = 4,705. `org_only_npi` does NOT appear at location level (org NPIs collapse into provider-keyed locations) — location-level corporate/independent shares are unaffected by the NPI-row org_only_npi count.

**Corporate share NPI vs location is structural.** NPI rows: 1,178 corporate / 13,818 watched = 8.53% (SQLite = Supabase, synced 2026-06-10). Locations: 285 corporate / 4,970 GP = **5.73%** (CHI 266/4,608 = 5.77%, BOS 19/362 = 5.25%) — synced, live. One corporate location houses 2-5 NPIs. **Use `practice_locations` / `zip_scores.corporate_location_count` for headline KPIs.** Only quote NPI counts when the unit is "individual dentists working at corporate" (Job Market scoring). Legacy `ownership_status` is ~zero in SQLite — `entity_classification` (via `classifyPractice()` helper) is canonical in the Next.js frontend.

**This 5.73% is a documented FLOOR, not the true corporate share.** It counts only locations with hard evidence (known DSO brand, corporate parent_company, EIN shared across 3+ ZIPs, or a Data-Axle structural signal cross-validated by an already-corporate EIN member / free WebSearch). DSOs routinely keep the acquired practice's local name, which name/EIN matching cannot see — so the true share is higher. The honest upper anchor is the **ADA HPI per-dentist DSO-affiliation rate** (a different unit — people, not locations): IL 2024 = **14.6%**, MA 2024 = **14.9%**. The frontend presents both as a band (`consolidation-honesty.ts` → `getCorporateBand`/`corporateBandTooltip`/`corporateBandSubtitle`), wired into Home, Market Intel, Job Market, and Warroom Sitrep + briefing. **Never present the confirmed floor as "the consolidation rate," and never fabricate a precise "true" number between the floor and the anchor.**
