...
> 🧮 **2026-07-09 — THE COUNTS IN THIS FILE ARE MACHINE-AUDITED; RUN THE DRIFT CHECK BEFORE QUOTING ANY OF THEM.**
> `python3 .claude/skills/dental-pe-skill-drift-check/check_claims.py` re-verifies every volatile
> number in this file and the `dental-pe-*` skills against the live DB (read-only; manifest =
> `.claude/skills/dental-pe-skill-drift-check/claims.json`; CI runs the file-only subset via
> `.github/workflows/skill-drift.yml`). The cheat-sheet, table rows, and classification
> breakdowns below were refreshed 2026-07-09 from live queries: GP universe **4,801** (IL 4,439 +
> MA 362), floor **268 locations / 1,152 watched NPIs = 5.58% / 8.34%** (guarded totals held;
> internal splits moved — locations 186 dso_national + 82 dso_regional, NPIs 680 dso_regional +
> 472 dso_national), deals **527** after the 2026-07-03 unverifiable-row purge (`d2777ec`),
...
> 🏁 **2026-07-04 — LANE A WAVE 1 LANDED + SUPABASE SYNCED BOTH LEGS + LIVE-APP VERIFIED; CENSUS
> COVERAGE 71.64% IS LIVE.** All 277 §6h blockers carry researched pure-Opus adjudications (28/28
...
> **2,837 locations** → totals **3,180/4,439 = 71.64% reviewed** (T1 1,471 / T2 934 /
> T3 537 / T4 28 / T5 151 / T6 59; pe_backed 118; NPI mirror 6,754). Detector floor UNTOUCHED
> (268/1,152/4,801). **Supabase sync EXECUTED under explicit user grant 2026-07-04 + read-back
> verified both legs EXACT** (leg 1 `_sync_floor_tables_only` 5,657 pl rows; leg 2
> `_sync_census_columns_practices` 6,754/6,754; floor still 268/1,152 live) — details in runbook
> **§6m** (`RESEARCH_HOME/MASTER_RESUME_LANE_A_FLEET_20260702.md`). Live app renders the census
> layer end-to-end (`/practice/<id>` shows tier + evidence, detector demoted to context).
> Remaining census queue: 649 triage (INCLUDES the 91 holds) + ~477 undetermined + ~610
> synthetic/wave-5 rows. **Next mission (truth-safe app redesign) is chartered in
...
| 4 | **4,801** | GP locations | 290 watched | `SUM(zip_scores.total_gp_locations)` (live 2026-07-09; was 4,811 post-2026-06-12 `da_unverified` purge, 4,970 before) | Home, Market Intel, Job Market, Launchpad headline |
| 5 | **4,439** | GP loc | CHI (269 IL) | watched + IL filter on (4); was 4,449 (2026-06-12), 4,608 (pre-purge) | Warroom Sitrep, Market Intel CHI; ALSO the census denominator (3,180/4,439 = 71.64%) |
| 6 | **362** | GP loc | BOS (21 MA) | watched + MA filter on (4); 4,439+362=4,801 ✅ | Boston surfaces |
| 7 | **4,439** | GP loc | all-IL (practice_locations) | `practice_locations` `state='IL'`+GP filter (excl. specialist/non_clinical/da_unverified/duplicate_location); was 4,448 (2026-06-12). Now EQUALS (5) — the old ±1 was one location with an IL watched zip but state≠IL | Dentagraphics-comparable scope |
| 8 | **4,435** | active-GP | all-IL, drop `solo_inactive` | (7) minus 4 contactless (solo_inactive collapsed 167→4 — the rest were DA synthetics) | Honest active-GP claim |
| 9 | **3,961** | (their unit) | all-IL | Dentagraphics infographic (F30) | External benchmark — +12.1% gap to (7); the purge closed most of the old +16.8% gap |
| 10 | **1,152** | corp NPIs | 13,818 watched | `entity_classification IN (dso_regional,dso_national)` (680 dso_regional + 472 dso_national, live 2026-07-09; was 628+491=1,119 at 2026-06-12, 1,178 pre-demotions); Supabase read-back last verified at the 2026-07-04 sync | NPI-row corporate KPI = **8.34%** |
| 11 | **268** | corp locations | 4,801 GP watched | `zip_scores.corporate_location_count` (live 2026-07-09); was 261 (2026-06-12 demotions), 285 (pre-demotions), 262 (pre Phase-4), 200 (pre IL-DSO-seed). GUARDED: CI `FLOOR expect_min=268` | Headline corporate share = **5.58%** (floor) |

**Most-confused comparison:** Job Market ~13.8k "practices" (NPI rows, line 2) vs Warroom Sitrep ~5,478 "Practices in Scope" (all-class locations minus da_unverified, line 3). Both correct under their own units. Conversion = structural NPPES dual-emission. Headline corporate/independent KPIs use the **GP-location** denominator (line 4 = 4,801), not all-class locations and not NPI rows.

**Dentagraphics gap (F30):** Their **3,961** for IL has only "data courtesy of Medicaid.gov" + "as is, with all faults" disclaimer; no methodology disclosed. The +12.1% gap (our 4,439 vs their 3,961, post-purge) cannot be assigned — notably the 2026-06-12 `da_unverified` purge closed most of the old +16.8% gap, suggesting much of it WAS our Data-Axle synthetics. Our number has SQL provenance; theirs has disclaimer. Don't claim "we match Dentagraphics" or "they're correct and we're wrong."
...
| `practice_locations` | 5,657 watched rows (4,801 GP: CHI 4,439 + BOS 362; +179 `da_unverified` excluded from all denominators) | Address-deduped, PK `location_id`. **All Sitrep KPIs + headline corp %/independent % source HERE — NOT `practices`.** Joined via `practice_to_location_xref`. Post-2026-06-12 purge (GP was 4,970) |
| `deals` | 527 | gdn 357 + pesp 153 + beckers 14 + beckers+gdn 3, live 2026-07-09. **Was 2,975 until the 2026-07-03 unverifiable-row purge** (`d2777ec`, `scrapers/cleanup_low_quality_deals.py`, evidence `data/dso_research/deal_quality_cleanup_20260703.json`) — pitchbook rows all purged. Column is `source`, NOT `data_source`. Drift reconciled `ac2140a` — Pass 2 of `_reconcile_deals()` keys NULL-target rows by composite hash. Oct 2020 – 2026 |
...
**Watched-ZIP `entity_classification` breakdown (NPI rows, SQLite live 2026-07-09):** small_group 2,774 / solo_established 2,628 / large_group 2,151 / specialist 1,424 / family_practice 1,325 / solo_high_volume 845 / non_clinical 742 / dso_regional 680 / org_only_npi 572 / dso_national 472 / da_unverified 179 / solo_new 17 / duplicate_location 5 / solo_inactive 4. NULL=0. Corporate NPIs = 680+472 = **1,152** (was 628+491=1,119 at 2026-06-12; the total is guarded while the internal split moves with promotions/demotions). Supabase read-back last verified at the 2026-07-04 sync (both legs exact) via surgical `scrapers/_sync_practices_changed_rows.py` (per-row UPDATE of rows changed since cutoff — avoids the destructive `watched_zips_only` TRUNCATE CASCADE); CI guard `FLOOR_NPI` (expect_min=1152) watches live. Raw-SQL flip scripts MUST bump `updated_at` (root-fixed in `reclassify_verified_corporate_il.py`).

**Location-level breakdown (`practice_locations`, watched, SQLite live 2026-07-09):** solo_established 2,440 / small_group 926 / specialist 648 / solo_high_volume 631 / large_group 309 / family_practice 205 / dso_national 186 / da_unverified 179 / dso_regional 82 / non_clinical 23 / solo_new 19 / duplicate_location 5 / solo_inactive 4. Total = 5,657 rows. Corporate locations = 186+82 = **268** (was 188+73=261 at 2026-06-12; guarded total, moving split). Independent (7 classes) = 4,534. `org_only_npi` does NOT appear at location level; `da_unverified` (179) and `duplicate_location` (5) DO but are excluded from every denominator and every UI surface.

**Corporate share NPI vs location is structural.** NPI rows: 1,152 corporate / 13,818 watched = 8.34% (SQLite live 2026-07-09). Locations: 268 corporate / 4,801 GP = **5.58%** (CHI 249/4,439 = 5.61%, BOS 19/362 = 5.25%). One corporate location houses 2-5 NPIs. **Use `practice_locations` / `zip_scores.corporate_location_count` for headline KPIs.** Only quote NPI counts when the unit is "individual dentists working at corporate" (Job Market scoring). Legacy `ownership_status` is ~zero in SQLite — `entity_classification` (via `classifyPractice()` helper) is canonical in the Next.js frontend.

**This 5.58% is a documented FLOOR, not the true corporate share.** It counts only locations with hard evidence (known DSO brand, corporate parent_company, EIN shared across 3+ ZIPs, or a Data-Axle structural signal cross-validated by an already-corporate EIN member / free WebSearch). DSOs routinely keep the acquired practice's local name, which name/EIN matching cannot see — so the true share is higher. The honest upper anchor is the **ADA HPI per-dentist DSO-affiliation rate** (a different unit — people, not locations): IL 2024 = **14.6%**, MA 2024 = **14.9%**. The frontend presents both as a band (`consolidation-honesty.ts` → `getCorporateBand`/`corporateBandTooltip`/`corporateBandSubtitle`), wired into Home, Market Intel, Job Market, and Warroom Sitrep + briefing. **Never present the confirmed floor as "the consolidation rate," and never fabricate a precise "true" number between the floor and the anchor.**
